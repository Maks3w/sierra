...
export const getStopsInformation = async (stopId: string): Promise<Stop> => {
  const response = await fetch(`https://www.crtm.es/widgets/api/GetStopsInformation.php?codStop=${stopId}`);
...
export const getStopsTimes = async (stopId: string): Promise<StopTimes> => {
  const response = await fetch(`https://www.crtm.es/widgets/api/GetStopsTimes.php?codStop=${stopId}&type=1&orderBy=2&stopTimesByIti=3`);
